...
def inputVariables(x):
    """
    Extract the desired variables given a line from the datafiles
    """
    ...
    lst = [structure_number, state, year_built, year_reconstructed,
           structure_length, average_daily_traffic]
    return lst
...
for year in years:
    ...
    for file in files:
        ...
        with open(fname, "rt", encoding="latin1") as f_in, gzip.open(fname
                  + ".gz", "wt", encoding="utf8") as f_out:
            shutil.copyfileobj(f_in, f_out)
        os.remove(fname)

# File maniputation
dpath = ("C:/Users/Akash Rastogi/Desktop/UMich study material/Fall 2016/" +
         "STATS 607/Assignments")
folders = os.listdir(dpath)
folders = [x for x in folders if x.startswith('20')]


# Extracting all the values for year = 2000 (i.e the 0th element in
# folders list)
yearDict = {}
...
lenOfBridgeDict = {}
for item in lYear2000:
    if not math.isnan(item[4]) and not math.isnan(item[1]):
        str_item = str(int(item[1]))
...
avgDuration = []
for item in lYear2000:
    if not math.isnan(item[2]) and not math.isnan(item[3]) and item[3] != 0.0:
...
yr2000dT = {i[0]: i[5] for i in yearDict['2000'] if not math.isnan(i[5])}
yr2010dT = {i[0]: i[5] for i in yearDict['2010'] if i[0] in yr2000dT.keys()
            and not math.isnan(i[5])}
...
# What was the average percentage change in average daily traffic over
# all bridges

(sum(yr2010dT.values()) - sum(yr2000dT.values())) / sum(yr2000dT.values())
# 0.1359
